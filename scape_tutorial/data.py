...
def generate_rows(day,seed=None):
    ...
    for u in users:
        ...
        for i in range(hits):
            ...
            event['src_ip'] = u['ip']
            event['user'] = u['user']
            dest = domains[pick_index([x['pop']**2 for x in domains])]
...
def generate_csv(day, seed=None):
    dhcp, addc, web = generate_rows(day, seed)
    for (name,rows) in [('dhcp.csv',dhcp),('addc.csv',addc),('web.csv',web)]:
        cols = sorted(rows[0].keys())
        with open(name,'w') as wfp:
            writer = csv.DictWriter(wfp, cols)
            writer.writeheader()
